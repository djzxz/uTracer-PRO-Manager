#!/usr/bin/env python3
"""Build database v2.27.0 from v2.26.0.

The migration materialises 99 unique remaining manufacturer/type entries that
already have a READY recommendation to an approved, source-verified profile and
an exact catalog type contained in that profile. This batch deliberately does
not claim a new independent PDF review: target-card identity stays marked as
not rechecked. It does not invent electrical values or promote CATALOG_ONLY
rows. Every new row copies execution values 1:1, is marked PASUJE DO, disables
...
SOURCE_DB = ROOT / "BAZA_DANYCH_v2_26_0" / "tube_measurements.db"
OUTPUT_DIR = ROOT / "BAZA_DANYCH_v2_27_0"
OUTPUT_DB = OUTPUT_DIR / "tube_measurements.db"
FULL_DB = OUTPUT_DIR / "tube_measurements_v2_27_0_FULL_SCHEMA_V7.db"
BATCH_SIZE = 99
RELEASE = "2.27.0"
...
def select_candidates(connection: sqlite3.Connection) -> list[sqlite3.Row]:
    rows = connection.execute(
        ...
        WHERE r.decision='READY'
          AND r.recommended_profile_id NOT GLOB 'MFR25_*'
          AND r.recommended_profile_id NOT GLOB 'MFR26_*'
          AND r.recommended_profile_id NOT GLOB 'MFR27_*'
    ...
    if len(eligible) < BATCH_SIZE:
        raise RuntimeError(
            f"Only {len(eligible)} exact-type existing-READY candidates; {BATCH_SIZE} required."
        )
...
def insert_profile(
    ...
    new_id = (
        f"MFR27_{safe_id(source['tube_type'])}_{safe_id(source['manufacturer'])}_{digest}"
    )
    ...
    values.update(
        {
            ...
            "source_title": (
                f"{source['manufacturer']} {source['tube_type']} — alias katalogowy; "
                f"zweryfikowane źródło wartości 1:1: {source['source_title']}"
            ),
            "source_url": source["source_url"],
            "source_page": (
                f"Docelowa karta katalogowa (tożsamość niezweryfikowana ponownie): "
                f"{source['data_sheet_url']} [{source['file_name']}]. Wartości wykonawcze "
                f"ze zweryfikowanego źródła profilu {template_id}: {source['source_page']}"
            ),
            "extraction_status": "READY_MANUFACTURER_ALIAS_EXISTING_RECOMMENDATION_V2_27",
            "approved_for_hardware": 1,
            "counts_for_condition_percent": 0,
            "notes": (
                f"{source['notes']}\n[v2.27.0] Osobny wariant producenta/karty utworzono "
                ...
                "Tożsamości i treści docelowej karty producenta nie weryfikowano "
                "ponownie w tej partii; wymagane jest "
    ...
    connection.execute(
        ...
        (
            source["datasheet_id"],
            new_id,
            f"PROFIL PRODUCENTA v2.27.0 — PASUJE DO {template_id}; wartości 1:1",
        ),
    )
    connection.execute(
        """
        ...
        SET recommended_profile_id=?,
            confidence='READY_MANUFACTURER_ALIAS_V2_27',
    ...
    connection.execute(
        ...
        (
            ...
            source["file_name"],
            "EXISTING_READY_RECOMMENDATION_TARGET_CARD_NOT_RECHECKED",
            0,
            0,
            "EXACT_CATALOG_TYPE_EXISTING_READY_RECOMMENDATION",
            "READY_MANUFACTURER_ALIAS_EXISTING_RECOMMENDATION_V2_27",
            json.dumps(
                [
                    "exact_catalog_type",
                    "existing_READY_recommendation",
                    "exact_catalog_type_contained_in_profile",
            ...
            json.dumps(
                [
                    "target_card_identity_recheck_v2_27",
                    "independent_target_pdf_content_recheck_v2_27",
                    "local_target_pdf_sha256",
                ]
            ),
            0,
            (
                f"v2.27.0: profil producenta materializuje istniejącą rekomendację READY "
                "z dokładnym oznaczeniem katalogowym zawartym w profilu. "
                f"PASUJE DO {template_id}; parametry skopiowano 1:1. Nie jest to nowa, "
                "niezależna weryfikacja tożsamości ani treści docelowej karty PDF. "
                "Ocena procentowa wyłączona; "
                "potwierdzenie operatora obowiązkowe."
            ),
            generated_utc,
            "EXACT_CATALOG_TYPE__TARGET_CARD_NOT_RECHECKED_V2_27",
        ),
    )
    connection.execute(
        """
...
def update_catalog_info(connection: sqlite3.Connection, generated_utc: str) -> None:
    ...
    set_info(connection, "batch_2_27_manufacturer_profiles_added", BATCH_SIZE)
    set_info(connection, "batch_2_27_values_copied_1_to_1", BATCH_SIZE)
    set_info(connection, "batch_2_27_percent_grade_disabled", BATCH_SIZE)
    set_info(connection, "batch_2_27_manual_confirmation", BATCH_SIZE)
    set_info(connection, "batch_2_27_target_card_recheck_missing", BATCH_SIZE)
    set_info(connection, "catalog_stage", "MANUFACTURER_READY_ALIASES_AND_REFERENCE_CURVES")
    set_info(
        connection,
        "release_label",
        f"v{RELEASE} — 99 unikalnych profili producent/typ PASUJE DO; łącznie {ready_profiles} READY",
    )
    set_info(
        connection,
        "verification_note",
        "v2.27.0: 99 unikalnych wariantów producent/typ utworzono wyłącznie z "
        "istniejących rekomendacji READY i dokładnego oznaczenia katalogowego zawartego "
        "w zatwierdzonym profilu. Wartości wykonawcze skopiowano 1:1 ze zweryfikowanego "
        "profilu; tożsamości i treści docelowych kart nie sprawdzano ponownie. Ocena "
        "procentowa wyłączona, potwierdzenie operatora obowiązkowe.",
    )


def write_outputs(connection: sqlite3.Connection, added: list[dict[str, object]]) -> None:
    csv_path = OUTPUT_DIR / "NOWE_PROFILE_PRODUCENTOW_99.csv"
    with csv_path.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(added[0]))
        writer.writeheader()
        writer.writerows(added)

    (OUTPUT_DIR / "NOWE_PROFILE_PRODUCENTOW_99.json").write_text(
    ...
    with (OUTPUT_DIR / "STATUS_BAZY_v2_27_0.csv").open(
    ...
    report = (
        "WERYFIKACJA BAZY uTracer PRO Manager v2.27.0\n"
        "================================================\n"
        ...
        "Zasada partii: dokładne oznaczenie katalogowe zawarte w istniejącym profilu, "
        "rekomendacja READY, zapis weryfikacji źródła i nieblokująca macierz sprzętowa. "
        "Parametry i macierz "
        ...
        "procentowej kondycji. Tożsamości ani treści docelowych kart producentów nie "
        "weryfikowano ponownie w tej partii; ten brak jest jawnie zapisany w bazie.\n"
    )
    (OUTPUT_DIR / "WERYFIKACJA_v2_27_0.txt").write_text(report, encoding="utf-8")
    readme = (
        "BAZA DANYCH v2.27.0 — PODMIANA\n"
        "================================\n"
        ...
        "Nowość: 99 kolejnych, unikalnych profili producent/typ oznaczonych PASUJE DO. "
        "Wartości pochodzą 1:1 z istniejących profili READY; docelowych kart nie "
        "weryfikowano ponownie, dlatego pomiar wymaga potwierdzenia operatora i nie "
        "wylicza procentowej kondycji. "
...
def main() -> None:
    ...
    print(f"Added {BATCH_SIZE} manufacturer/type profiles")
