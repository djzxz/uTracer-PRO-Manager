#!/usr/bin/env python3
"""Build database v2.26.0 from v2.25.0.

The migration materialises every remaining strict manufacturer/card-specific
measurement profile (175 in the v2.25.0 input)
...
SOURCE_DB = ROOT / "src" / "uTracerProManager.Avalonia" / "Data" / "tube_measurements.db"
OUTPUT_DIR = ROOT / "BAZA_DANYCH_v2_26_0"
OUTPUT_DB = OUTPUT_DIR / "tube_measurements.db"
FULL_DB = OUTPUT_DIR / "tube_measurements_v2_26_0_FULL_SCHEMA_V7.db"
BATCH_SIZE = 175
RELEASE = "2.26.0"
...
def insert_profile(
    ...
    new_id = (
        f"MFR26_{safe_id(source['tube_type'])}_{safe_id(source['manufacturer'])}_{digest}"
    )
    ...
    values.update(
        {
            ...
            "extraction_status": "READY_MANUFACTURER_ALIAS_EXISTING_RECOMMENDATION_V2_26",
            "approved_for_hardware": 1,
            "counts_for_condition_percent": 0,
            "notes": (
                f"{source['notes']}\n[v2.26.0] Osobny wariant producenta/karty utworzono "
    ...
    connection.execute(
        ...
        (
            source["datasheet_id"],
            new_id,
            f"PROFIL PRODUCENTA v2.26.0 — PASUJE DO {template_id}; wartości 1:1",
        ),
    )
    connection.execute(
        """
        ...
        SET recommended_profile_id=?,
            confidence='READY_MANUFACTURER_ALIAS_V2_26',
    ...
    connection.execute(
        ...
        (
            ...
            "READY_MANUFACTURER_CARD_ALIAS_EXISTING_RECOMMENDATION_V2_26",
            json.dumps(
                [
                    "exact_catalog_type",
            ...
            json.dumps(
                ["independent_pdf_content_recheck_v2_26", "local_pdf_sha256"]
            ),
            0,
            (
                f"v2.26.0: profil producenta materializuje istniejącą rekomendację READY. "
    ...
    connection.execute(
        ...
        (
            ...
            "EXACT_CATALOG_TYPE_FILENAME__CONTENT_NOT_RECHECKED_V2_26",
            0,
            generated_utc,
        ),
    )
...
def update_catalog_info(connection: sqlite3.Connection, generated_utc: str) -> None:
    ...
    set_info(connection, "batch_2_26_manufacturer_profiles_added", BATCH_SIZE)
    set_info(connection, "batch_2_26_values_copied_1_to_1", BATCH_SIZE)
    set_info(connection, "batch_2_26_percent_grade_disabled", BATCH_SIZE)
    set_info(connection, "batch_2_26_manual_confirmation", BATCH_SIZE)
    set_info(connection, "catalog_stage", "MANUFACTURER_READY_ALIASES_AND_REFERENCE_CURVES")
    set_info(
        connection,
        "release_label",
        f"v{RELEASE} — 175 profili producentów PASUJE DO; łącznie {ready_profiles} READY",
    )
    set_info(
        connection,
        "verification_note",
        "v2.26.0: 175 pozostałych ścisłych wariantów producent/karta utworzono wyłącznie z "
...
def write_outputs(connection: sqlite3.Connection, added: list[dict[str, object]]) -> None:
    csv_path = OUTPUT_DIR / "NOWE_PROFILE_PRODUCENTOW_175.csv"
    ...
    with (OUTPUT_DIR / "STATUS_BAZY_v2_26_0.csv").open(
    ...
    report = (
        "WERYFIKACJA BAZY uTracer PRO Manager v2.26.0\n"
        "================================================\n"
    ...
    (OUTPUT_DIR / "WERYFIKACJA_v2_26_0.txt").write_text(report, encoding="utf-8")
    readme = (
        "BAZA DANYCH v2.26.0 — PODMIANA\n"
        "================================\n"
        ...
        "Nowość: 175 pozostałych ścisłych profili producent/karta oznaczonych PASUJE DO. "
